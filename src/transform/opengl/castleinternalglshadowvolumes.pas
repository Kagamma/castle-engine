...
begin
  ...
  if {$ifndef FPC_WEB}(not Assigned(glStencilOpSeparate)) and{$endif} Load_GL_ATI_separate_stencil then
  begin
  ...
  FStencilTwoSided := {$if defined(CASTLE_NINTENDO_SWITCH) or defined(FPC_WEB)} true {$else} {$ifndef FPC}@{$endif}glStencilOpSeparate <> nil {$endif};
